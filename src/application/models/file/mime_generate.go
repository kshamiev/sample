...
const (
	...
	dataFormatIanaCSV     = `csv`   // Вортам данных https://www.iana.org/assignments/media-types/media-types.xhtml
)

var (
	rexExtension    = regexp.MustCompile(`\.([^\.]+)$`)
	rexSpliter      = regexp.MustCompile(`[\s\W]`)
...
func Main() (ret int) {
	...
	var mimeTypes, tmp []*MimeTypeItem
	var deduplicate map[string]*MimeTypeItem
	var fh *os.File
	...
	for i = range dirContent {
		...
		default:
			log.Warningf("unknown format %q, skipped", ext)
			continue
		}
...
func CreateConstantName(data map[string]*MimeTypeItem) {
	...
	for tn = range data {
		...
		data[tn].Name = preffix + strings.Join(tmp, delimiter)
	}
}
