...
func (d *decoder) readHeader() (err error) {
	...
	err = binary.Read(d.r, binary.LittleEndian, &first)
	err = binary.Read(d.r, binary.LittleEndian, &second)
	err = binary.Read(d.r, binary.LittleEndian, &d.num)
	if first != 0 {
...
func (d *decoder) setupBMP(e entry, data []byte) ([]byte, []byte, int, error) {
	...
	err = binary.Read(bytes.NewReader(img[14:14+4]), binary.LittleEndian, &dibSize)
	err = binary.Read(bytes.NewReader(img[14+4:14+8]), binary.LittleEndian, &w)
	...
	err = binary.Read(bytes.NewReader(img[14+32:14+36]), binary.LittleEndian, &numColors)
