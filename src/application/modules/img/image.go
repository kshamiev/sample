...
import (
	...
	"application/modules/img/ico" // Расширение image для формата ico
...
// Write Реализация интерфейса io.WriterTo
func (i *imgItem) WriteTo(w io.Writer) (n int64, err error) {
	if w == nil {
		err = fmt.Errorf("io.Writer is nil")
		return
	}
	if i.image == nil {
		err = fmt.Errorf("Image is nil, no data to write")
		return
	}
	switch i.tpe {
	case TypeICO:
	...
	default:
		err = fmt.Errorf("Incorrect type is %q", i.tpe.String())
	}
